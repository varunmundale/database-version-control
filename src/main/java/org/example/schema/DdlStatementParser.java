...
/**
 * Converts raw {@code CREATE TABLE} / {@code ALTER TABLE ... ADD|DROP [COLUMN]} statements directly into the
 * internal {@link TableModel} representation, using the same stable-id scheme as {@link AbstractJdbcSchemaParser}.
 * Table-level constraints and indexes are not modeled; only tables and columns are.
 */
public final class DdlStatementParser {
    ...
    private static IllegalArgumentException unsupported(String statement) {
        return new IllegalArgumentException(
                "Unsupported DDL statement (only CREATE TABLE and ALTER TABLE ADD|DROP COLUMN are supported): " + statement);
    }
}
