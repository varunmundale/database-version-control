...
public final class PostgresDdlParser implements DdlParser {
    ...
    private SchemaOperation toCreateTable(CreateTable createTable) {
        ...
        List<ColumnDefinition> columns = createTable.getColumnDefinitions().stream()
                .map(PostgresDdlParser::toColumnDefinition)
                .toList();
    ...
    private SchemaOperation toAlterOperation(Alter alter, String ddl) {
        ...
        return switch (expression.getOperation()) {
            case ADD -> new SchemaOperation.AddColumn(tableName, toColumnDefinition(soleColumn(expression, ddl)));
    ...
    private static ColumnDefinition toColumnDefinition(net.sf.jsqlparser.statement.create.table.ColumnDefinition column) {
        ...
        return new ColumnDefinition(name, type, !notNull, defaultValueOf(specs));
    }
