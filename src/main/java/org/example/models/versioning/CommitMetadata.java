 ...
 * derives. Supplied by the command line ({@code dbgit commit -m "..." [--author <name>]}) and defaulted here when
 * it isn't, so no caller has to decide what an absent author or message means.
 */
public record CommitMetadata(String author, String message) {
    ...
    public static CommitMetadata by(String author, String message) {
        return new CommitMetadata(author == null ? System.getProperty("user.name") : author, message);
    }
}
