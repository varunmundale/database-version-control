...
/**
 * Who is asking, which branch they are on, and how to reach the database {@code main} tracks - everything the
 * daemon used to read from its own working directory and now receives per request instead.
 *
 ...
 * each user reaches the tracked database as themselves.
 *
 * @param trackedDatabase {@code null} until the client's workspace has run {@code dbgit init}
 */
public record RequestContext(String user, String branch, ConnectionSettings trackedDatabase) {
    public static final String DEFAULT_BRANCH = "main";
    private static final String UNKNOWN_USER = "unknown";

    public RequestContext {
        user = user == null || user.isBlank() ? UNKNOWN_USER : user.strip();
    ...
    public RequestContext onBranch(String otherBranch) {
        return new RequestContext(user, Objects.requireNonNull(otherBranch, "branch must not be null"), trackedDatabase);
    }
}
