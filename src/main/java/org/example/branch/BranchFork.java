...
public final class BranchFork {
    ...
    public BranchFork(CommandRunner commandRunner) {
        this(commandRunner, defaultConnectorFactory(BranchDatabaseConfig.getInstance()), defaultMetadataStore());
    }

    public BranchFork(CommandRunner commandRunner, ConnectorFactory connectorFactory) {
    ...
    public SqlConnector connect(String database) throws SQLException {
        return connectorFactory.connect(config.connectionTo(database));
    }
    ...
    public BranchForkResult fork(String fromBranch, String currentBranch) {
        ...
        recreateDatabase(database, history);
    ...
    private void recreateDatabase(String database, List<ChangeSet> history) {
        try (SqlConnector connector = connect(database)) {
            databaseRecreator.recreate(connector, history);
        } catch (SQLException exception) {
            throw fail("Could not recreate branch database '" + database + "': " + exception.getMessage(), exception);
        }
    }
    ...
    private static ConnectorFactory defaultConnectorFactory(BranchDatabaseConfig config) {
        return ConnectorRegistry.builtins().get(config.dialect());
    }

    private static BranchMetadataStore defaultMetadataStore() {
        MetadataStoreConfig metadataConfig = MetadataStoreConfig.getInstance();
        ConnectorFactory factory = ConnectorRegistry.builtins().get(metadataConfig.dialect());
